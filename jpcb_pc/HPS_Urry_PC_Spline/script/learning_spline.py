...
#SBATCH --array=0-5
#SBATCH --output=./slurm_output/PC_%a.out
...
def get_data(protein_name):
    with open(f"./output/basis/{protein_name}.pkl", "rb") as file_handle:
...
for idx_protein in range(len(names)):
    if not os.path.isfile(f"./output/basis/{names[idx_protein]}.pkl"):
        continue
...
with open('./output/basis/omega.pkl', 'rb') as file_handle:
    omega = pickle.load(file_handle)
...
with open('./output/basis/init_theta.pkl', 'rb') as file_handle:
...
def compute_loss_and_grad(x, wc):
    ...
    theta = x[0:-len(data_list)]
    theta = theta.reshape((210, 13))
    loss += 0.5*wc*np.sum(np.matmul(theta, omega) * theta)

    grad_theta = np.array(grad_theta)
    grad_theta += wc*np.matmul(theta, omega).reshape(-1)

    grad_F = np.array(grad_F)

    grad = np.concatenate([grad_theta, grad_F])
    return loss, grad



# wc_list = [1e-10, 1e-9, 1e-8, 1e-7, 1e-6, 1e-5]
...
with open(f'./output/params/params_optimized_wc_{wc:.2E}.pkl', 'wb') as f:
    pickle.dump(results.x[0:-len(data_list)].reshape((210, 13)), f)
