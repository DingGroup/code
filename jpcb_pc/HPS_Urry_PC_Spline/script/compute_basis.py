...
def compute_basis(traj):
    basis = defaultdict(lambda: np.zeros((traj.n_frames, 13)))
    intercept = 0
...
with open(f"./output/basis/{protein_name}.pkl", "wb") as file_handle:
    pickle.dump({
...
if idx_protein == 1:    
    _, omega = spline.bs_nb(np.zeros(10), 2.0, 12, omega = True)
    omega = omega / (kbT.value_in_unit(unit.kilojoule_per_mole))**2
    with open(f"./output/basis/omega.pkl", "wb") as file_handle:
        pickle.dump(omega, file_handle)
